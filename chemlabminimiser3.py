                    ...
                    if self.type_array[hmbc[0]] == "H":
                        ...
                        if self.free_valencies[carbon_index]!=0:
                            ...
                            if len(adjacent_carbons) == 1:
                                self.merge(frag1,frag2,[adjacent_carbons[0], hmbc[1]])
        for fragment in self.fragments:
...
class Fragment:
    ...
    def set_renderer(self):
        self.renderer = self.viewer.add_renderer(BallAndStickRenderer, self.coordinate_manager.atom_coordinates[self.global_indices], self.type_array[self.global_indices], np.array(self.local_bond_indices))
