...
class FileManager:
    ...
    def numpy_to_xyz_string(self, filename, best_atom_coordinates):
        ...
        signal_info = self.interaction_manager.atom_types
        data_string = ""
        data_string += (str(self.interaction_manager.number_signals)+"\n")
        data_string += ("Optimised cartesian coordinates\n")
        for i in range(self.interaction_manager.number_signals):
            data_string+=(signal_info[i]+" "+str(best_atom_coordinates[i][0]/10)+" "+str(best_atom_coordinates[i][1]/10)+" "+str(best_atom_coordinates[i][2]/10)+"\n")
